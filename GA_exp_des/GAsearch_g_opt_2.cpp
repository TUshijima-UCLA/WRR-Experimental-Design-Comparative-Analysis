...
//////////// 1/6/13 stop modify code here
		...
		if (OptCI == 'G' || OptCI == 'I')
		{
			...
			red_solver(Pv,Ar,Br,well,wellrate,npc,Nn,nq,obstimes,dtmult,dibase,nprt,lumpwell,dt,Pi);
			...
			for (int ii = 0; ii < nz; ii++)
			{
				...
				red_solver(Pv,Ar,Br,well,wellrate,npc,Nn,nq,obstimes,dtmult,di,nprt,lumpwell,dt,Pi);
				//Calculate sensitivies to changes in hydraulic conductivty
				if (expdesparam == 1)
				{
					for (unsigned int jj = 0; jj < dibase.size(); jj++)
					{
						dI[ii*dibase.size() + jj] = dibase[jj] - di[jj]; 
					}
				}
				else
				{
