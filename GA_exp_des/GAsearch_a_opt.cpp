...
for (int gacurrentcall = 0; gacurrentcall < gatotalcalls; gacurrentcall++)
{
	...
	if (pickgenome == 1)	//set up first genome (new type)
	{
		...
		for (int ii = 0; ii < nobszones; ii++) {ub[ii] = ozonec[ii] - 1;}
		////Add nobzones gene alleles (# genes = nobszones)
		for (int ii = 0; ii < nobszones; ii++) {alleles.add(lb[ii],ub[ii],1,GAAllele::INCLUSIVE,GAAllele::INCLUSIVE);}
	}
	else if (pickgenome == 2)	//set up old type of genome
	{
		...
		for (int ii = 0; ii < nowell; ii++) {alleles.add(lb,ub,1,GAAllele::INCLUSIVE,GAAllele::INCLUSIVE);}
	}
...
//////////// 1/6/13 stop modify code here
		...
		*/
		...
		ga.statistics().maxEver() << " Current Convergence: " << ga.convergence() << "                         ";
		ga.flushScores();
		}
		std::cout << std::endl;
